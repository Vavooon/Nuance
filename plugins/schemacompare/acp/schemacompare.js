function SchemaCompare() {
	var bodyWrap = ce('div', {
		className: 'opts-tab schemacompare-tab'
	});

	var headerWrap = ce('div', {
		className: 'header-wrap'
	}, bodyWrap);
	ce('span', {
		className: 'current-table-name',
		innerHTML: _("Current table")
	}, headerWrap);
	ce('span', {
		className: 'dump-table-name',
		innerHTML: _("Dump table")
	}, headerWrap);
	ce('div', {
		innerHTML: _("Update"),
		id: 'schema-update-button',
		onclick: loadSchema
	}, headerWrap);
	var queryEl = ce('div', {
		className: 'query'
	...
	function displaySchema(resp) {
		...
		for (var tableName in dumpSchema) {
			...
			var currentTableContent = ce('div', {
				className: 'current-table-content'
			}, result);
			var dumpTableContent = ce('div', {
				className: 'dump-table-content'
			}, result);
			for (var column in currentTable) {
				...
				for (var prop in dumpTable[column]) {
					if (dumpTable[column][prop]) {
						...
						ce('div', {
							className: className,
							innerHTML: prop + ': ' + currentTable[column][prop]
						}, columnWrap2);
						ce('div', {
							className: className,
							innerHTML: prop + ': ' + dumpTable[column][prop]
						}, DcolumnWrap2);
					}
				}
			}
	...
	pluginsTabs.schemacompare = {
		title: _("Schema compare"),
		name: 'schemacompare',
		content: bodyWrap
	};
}
