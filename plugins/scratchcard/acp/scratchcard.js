		...
		tables.tabs.scratchcard = {
			...
			grid: {
				...
				toolbarButtons: [{
					...
					onclick: function() {
						var countField = new Nuance.input.TextField({
							name: 'count',
							title: _("Number of codes"),
							value: ""
						});
						var valueField = new Nuance.input.TextField({
							name: 'value',
							title: _("Values of codes"),
							value: ""
						});
						var showCodes = function(r) {
							new Nuance.CodesPopup({
								title: _("Generated scratchcards"),
								text: r.data.join('\n')
							});
							scStore.load();
						}
						var sendRequest = function() {
							if (!countField.getValue() || !valueField.getValue()) {
								new Nuance.MessageBox({
									title: _("Error"),
									text: _("Please fill out all fields")
								});
							} else {
								exportForm.close();
						...
						var exportForm = new Nuance.Popup({
							title: _("Export"),
							buttons: [{
								onclick: sendRequest,
								value: _("Generate codes")
							}],
							fields: [countField, valueField]
						});
					}
				}]
			}
