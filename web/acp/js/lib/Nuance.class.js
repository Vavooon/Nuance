...
var Nuance = {
	...
	ConfigProxy: function(o) {
		...
		this.setValue = function(type, path, name, value, owner) {
			...
			if (owner === undefined)
				return;
			if (!self.__config[type][owner])
				self.__config[type][owner] = {};
			if (!self.__config[type][owner][path])
				self.__config[type][owner][path] = {};
	...
	AjaxRequest: function(method, path, post, onsuccess, onerror, simpleResponse) {
		if (!path)
			throw new Error(_("Path must be specified"));
	...
	input: {
		...
		ButtonGroup: function(o) {
			this.body = ce('div', {
				className: 'buttons-wrap buttons-group'
			}, o.target);
			if (o.mergeButtons)
				this.body.classList.add('button-group');
			if (o.btnLayout)
				this.body.classList.add(o.btnLayout);
		...
		ProgressBar: function(o) {
			this.body = ce('div', {
				...
				innerHTML: o.title + '<br />'
			}, o.target);
			var self = this;
			self.el = ce('div', {
				className: 'progressbar field'
			}, this.body);
			var value = 0,
		...
		TextField: function(o) {
			this.body = ce('div', {
				className: 'text-field-wrap field-wrap'
			}, o.target);
			var flexWrap = ce('div', {
				className: 'flex-wrap'
			}, this.body);
		...
		SpeedField: function(o) {
			this.body = ce('div', {
				className: 'speed-field-wrap field-wrap'
			}, o.target);
			//var titleWrap = ce('span', {className: 'title-wrap'}, this.body);
			//this.title = ce('span', {className: 'title', innerHTML: o.title || ""}, titleWrap);
			this.wrap = ce('div', {
		...
		SearchField: function(o) {
			this.body = ce('div', {
				className: 'field-wrap search-wrap',
				innerHTML: o.title ? o.title + '<br />' : ""
			}, o.target);
			var self = this,
				disabled = false,
			...
			var clearButton = ce('div', {
				type: 'button',
				className: 'clear-button icon remove',
				disabled: true,
				onclick: clearSearch
			}, this.body);
			self.el = ce('input', {
			...
			self.el.onchange = self.el.onkeyup = function(e) {
				...
				if (e && e.type == 'keyup' && e.which == 13 && canSearch)
					searchhandler(self.getValue());
				if (e && e.type == 'keyup' && e.which == 27 && canSearch)
					clearSearch();
				if (e && !this.value)
					clearSearch();
			};
		...
		DateField: function(o) {
			...
			this.body = ce('div', {
				className: 'date-field-wrap field-wrap'
			}, o.target);
			//var titleWrap = ce('span', {className: 'title-wrap'}, this.body);
			this.wrap = ce('div', {
				className: 'flex-wrap'
			...
			var yearStore = new Nuance.MemoryStore({
				target: 'year',
				header: [
					['id', 'id'],
					['name', 'varchar']
		...
		PhoneNumberField: function(o) {
			this.body = ce('div', {
				className: 'phone-field-wrap field-wrap'
			}, o.target);
			//var titleWrap = ce('span', {className: 'title-wrap'}, this.body);
			//this.title = ce('span', {className: 'title', innerHTML: o.title || ""}, titleWrap);
			this.wrap = ce('div', {
		...
		PasswordField: function(o) {
			this.body = ce('div', {
				className: 'password-field-wrap field-wrap'
			}, o.target);
			this.wrap = ce('div', {
				className: 'flex-wrap'
			}, this.body);
		...
		ViewPasswordField: function(o) {
			this.body = ce('div', {
				className: 'view-password-field-wrap password-field-wrap field-wrap'
			}, o.target);
			//var titleWrap = ce('span', {className: 'title-wrap'}, this.body);
			//this.title = ce('span', {className: 'title', innerHTML: o.title || ""}, titleWrap);
			this.wrap = ce('div', {
			...
			var toggleViewButton = ce('div', {
				className: 'button icon onlyicon unlock view-password',
				onclick: toggleView
			}, this.wrap, true);
		...
		GenerateViewPasswordField: function(o) {
			...
			var self = this,
				generateButton = ce('div', {
					className: 'button icon onlyicon flash generate-password'
				}, this.wrap);
		...
		FileField: function(o) {
			...
			this.body = ce('div', {
				className: 'file-field-wrap field-wrap'
			}, o.target);
			this.title = ce('span', {
				className: 'title',
				innerHTML: o.title ? o.title : ""
			}, this.body);
			var self = this,
				value = o.value;
			this.setValue = function(value) {
				...
				if (this.value) {
					if (this.files[0].size > 8000000) {
						new Nuance.MessageBox({
							text: _("File size should be less than 8M")
						});
					} else {
			...
			var button = ce('div', {
				...
				innerHTML: value ? _("Select new file") : _("Select file")
			}, self.body);
			this.el = el;
			Nuance.input.__Field.call(this, o);
		},
		CheckBox: function(o) {
			this.body = ce('div', {
				className: 'checkbox-field-wrap field-wrap'
			}, o.target);
			this.wrap = ce('div', {
				className: 'flex-wrap'
			}, this.body);
		...
		ComboBox: function(o) {
			...
			if (o.multiple) {
				...
				var selectAllEl = ce('div', {
					className: 'select',
					innerHTML: _("Select all"),
					onclick: selectAll
				}, toolsContainer);
				var unselectAllEl = ce('div', {
					className: 'unselect',
					innerHTML: _("Unselect all"),
					onclick: unselectAll
				}, toolsContainer);
			}
			var self = this,
        ...
        if (trigger) {
          self.trigger('change', self.getValue());
			...
			function searchItem(e) {
				...
				{
					if (!hoverEl)
						hoverEl = (e.which == 40) ? optionsContainer.children[1] : optionsContainer.lastChild;
			...
			function loadOptions(array) {
				...
				if (!o.avoidSort) {
					elementsOptions.sort(function(a, b) {
				...
				if (values.length && o.showNotSelected) {
					var notSelOpt = ce('div', {
						className: 'option',
						innerHTML: _('Not selected'),
						value: 0,
						onclick: toggleOption,
						onselectstart: falsefunc,
						onsmousedrag: falsefunc
					...
					if (typeof plValue === 'object' && plValue.length > 1) {
						...
						if (this.children[0].nValue != nullValue) {
							...
							for (var option in store.data) {
								...
								for (var i = 0; i < value.length; i++) {
									if (value[i] == plValue || value[i] == 0)
										newOpts[option] = store.getNameById(option);
								}
							}
						} else {
							pListSelected = false;
			...
			function hover(e) {
				if (e.type == 'mouseover' && !self.isDisabled())
		...
		TariffComboBox: function(o) {
			...
			this.form = o.form;
			this.body = ce('div', {
				className: 'tariff-field-wrap field-wrap'
			}, o.target);
			this.wrap = ce('div', {
				className: 'flex-wrap'
			}, this.body);
			var tariffCombobox = new Nuance.input.ComboBox({
				name: 'tariff',
				store: o.store,
				value: o.value,
			...
			var changeButton = ce('div', {
				className: 'change-tariff-button button icon onlyicon arrowright',
		...
		Documents: function(o) {
			...
			function Document(o) {
				var self = this;
				this.body = ce('div', {
					className: "button document-button document"
				}, o.target);
				var title = ce('em', {
					innerHTML: o.name,
					className: 'title'
				...
				var renameButton = new Nuance.input.Button({
					onlyIcon: true,
					iconClass: 'edit',
					target: self.body,
					onclick: function() {
				...
				var removeButton = new Nuance.input.Button({
					onlyIcon: true,
					iconClass: 'remove',
			...
			function upload() {
				...
				popup.close();
				var loadingPopup = new Nuance.LoadingPopup({
					text: "Uploading..."
				});
				xhr.onload = function() {
					loadingPopup.close();
			...
			function docPopup(o) {
				...
				new Nuance.input.TextField({
					...
					title: _("Name"),
					target: form,
					value: index !== -1 ? documents[index].name : null
				});
				new Nuance.input.FileField({
					name: 'file',
					title: _("File"),
					target: form,
					value: index !== -1 ? documents[index].name : null
				});
				forceDownloadField = new Nuance.input.CheckBox({
					name: 'forceDownload',
					title: _("Force download"),
					target: form,
					value: index !== -1 ? documents[index].forceDownload : null
				});
				descriptionField = new Nuance.input.TextArea({
					name: 'comment',
					title: _("Description"),
					target: form,
					value: index !== -1 ? documents[index].description : null
				});
		...
		TextArea: function(o) {
			this.body = ce('div', {
				className: 'textarea-field-wrap field-wrap double'
			}, o.target);
			this.wrap = ce('div', {
				className: 'flex-wrap'
			}, this.body);
		...
		TariffList: function(o) {
			this.body = ce('div', {
				className: 'tariff-list field-wrap field-wrap'
			}, o.target);
			var titleWrap = ce('span', {
			...
			var self = this,
				disabled = false,
				addButton = new Nuance.input.Button({
					target: this.body,
					iconClass: 'add',
					onclick: function() {
						new TariffRow({
				...
				table = ce('div', {
					className: 'tarifflist field'
				}, this.body),
				rows = [];

			function TariffRow(o) {
				var row = ce('div', {
						className: 'tarifflist-row'
					}, o.target),
					disabled = false,
					selfRow = this;
				var timeField = new Nuance.input.TextField({
					value: o.starttime ? o.starttime : '',
					target: row
				});
				...
				var dlField = new Nuance.input.TextField({
					value: o.dl ? o.dl : '',
					target: row
				});
				var ulField = new Nuance.input.TextField({
					value: o.ul ? o.ul : '',
					target: row
				});
			}
		...
		AccountField: function(o) {
			var self = this;
			this.body = ce('div', {
				className: 'account-field-wrap field-wrap double'
			}, o.target);
			...
			function ipRow(o) {
				this.body = ce('div', {
					className: 'ip-field-wrap field-wrap double'
				}, o.target);

				var row = ce('div', {
					className: 'ip-row'
				}, this.body);
				var firstSubRow = ce('div', {
					className: 'ip-sub-row'
				}, row);
				var secondSubRow = ce('div', {
					className: 'ip-sub-row'
				}, row);
				var field = ipStore.getById(o.recordId, true);

				var ipField = new Nuance.input.TextField({
					value: field.ip4,
					target: firstSubRow
				});
				var toMacButton = new Nuance.input.Button({
					target: firstSubRow,
					iconClass: 'pin'
				});
				var macField = new Nuance.input.TextField({
					value: field.mac,
					target: firstSubRow
				});
				var deleteButton = new Nuance.input.Button({
					target: firstSubRow,
					onclick: this.remove,
					iconClass: 'remove'
				});

				var portField = new Nuance.input.TextField({
					value: field.port,
					target: secondSubRow
				});
				var routerField = new Nuance.input.TextField({
					value: field.router,
					target: secondSubRow
				});
			}
			...
			for (var i in ipStore.data) {
				...
				if (row[ipNs.user] == o.recordId) {
					ipRows.push(new ipRow({
						recordId: row[ipNs.id],
						target: this.body
					}));
				}
			}
		...
		IpList: function(o) {
			this.body = ce('div', {
				className: 'iplist-field-wrap field-wrap double'
			}, o.target);
			this.title = ce('span', {
				className: 'title',
				innerHTML: o.title ? o.title : ""
			...
			function IpRow(o) {
				var row = ce('div', {
						className: 'iplist-row'
					}, o.target),
					disabled = false,
					selfRow = this;

				if (o.type == 'mikrotikppp')
					ce('div', {
						innerHTML: o.title
					}, row);
				var ipField = new Nuance.input.TextField({
				...
				function macPress() {
					...
					if (routerId) {
						...
						Nuance.AjaxRequest("POST", "./ajax.php?action=routergetmac", {
							router: routerId,
							ip: ipField.getValue()
						}, updMac);
					} else {
			...
			function toggleMode() {
				...
				for (var prop in value) {
					new IpRow({
						title: (length ? _('Remote IP') : _('Local IP')),
						target: table,
						ip: prop,
						mac: value[prop],
						type: routerType
		...
		PermissionList: function(o) {
			var self = this,
				disabled = false;
			this.body = ce('div', {
				className: 'permissionlist-field-wrap field-wrap double'
			}, o.target);
			var titleWrap = ce('span', {
				className: 'title-wrap'
			}, this.body);
			...
			function loadAcl(aclMap, aclChapter, child, chapter, level, parentValue, checkboxTree) {
				for (var i in aclMap) {
					if (typeof aclMap[i] === 'object') {
						var newChild = ce('div', {
							className: 'child acl-' + level + ' acl-' + i
						}, child);
						...
						var checkbox = new Nuance.input.CheckBox({
							offText: ' ',
							onText: " ",
							partialText: " ",
							value: nextChapter,
			...
			for (var i = 0; i < options.length; i++) {
				...
				if (options[i].topSeparator && i) {
					ce('div', {
						...
						oncontextmenu: falsefunc
					}, this.el);
				}
				var itemEl = ce('div', {
					className: 'context-option',
					innerHTML: options[i].title,
					oncontextmenu: falsefunc
				}, this.el);
	...
	Popup: function(o) {
		...
		function addClasses() {
			...
			if (popups.length)
				popups[popups.length - 1].style.position = '';
		}
		this.close = function() {
			if (closeAnimation) {
		...
		if (o.target) {
			...
			this._bigPopupWraps = ce('div', {
				className: 'big-popups-wrap'
			}, document.body);
			this._popupWraps = ce('div', {
				className: 'popups-wrap'
			}, this._bigPopupWraps);
			this._popupWrap = ce('div', {
				className: 'popup-wrap'
			}, this._popupWraps);
			this._popupWin = ce('div', {
				className: 'popup-window'
			}, this._popupWrap);
			addClasses();
			if (o.winLayout)
				this._popupWin.classList.add(o.winLayout);
		}
		var popupTitle = ce('p', {
		...
		if (!o.target && o.closable != false) {
			var closeBtn = new Nuance.input.Button({
				target: popupTitle,
				onclick: this.close,
				iconClass: 'remove'
			});
		...
		this.body = (o.body) ? this._popupWin.appendChild(o.body) : ce('div', {
			className: 'popup-body',
			name: 'popup-body'
		}, this._popupWin);
		if (o.bodyLayout)
			this.body.classList.add(o.bodyLayout);
		var fieldsByName = {};
		...
		if (o.buttons) {
			...
			for (var i = 0; i < buttons; i++)
				buttons[i].scope = self;
			var buttons = new Nuance.input.ButtonGroup({
				buttons: buttons,
				btnLayout: o.btnLayout,
				target: this._popupWin
			}).getButtons();
		}
	},
	...
	PreferencesActivity: function(o) {
		...
		var saveMessage = ce('div', {
			className: 'save-message flex-wrap',
			style: 'display: none;'
		}, tabPanel.contentEl);
		var saveMessageText = ce('div', {
			className: 'save-message-text',
			innerHTML: _("Some settings were changed. Do you want to save it?")
		}, saveMessage);
		...
		var saveButton = new Nuance.input.Button({
			value: _("Save"),
			iconClass: 'ok',
			target: saveMessage,
			onclick: saveChanges
		});
		var cancelButton = new Nuance.input.Button({
			value: _("Cancel"),
			iconClass: 'remove',
			target: saveMessage,
		...
		this.getValues = function() {
			...
			for (var i = 0; i < fields.length; i++) {
				...
				if (index != -1)
					values[index] = fields[i].getValue();
			}
			return values;
		};
		...
		for (var n = 0; n < sections.length; n++) {
			...
			for (var name in section) {
				...
				var namedField = ce('div', {
					className: 'named-field'
				}, tabPanel.tabs[i]);
				ce('div', {
					className: 'title',
					innerHTML: _([configType, i, name].join('-'))
				}, namedField);
				var field;
				var fieldOpts = {
					name: name,
				...
				switch (type) {
					...
					case 'link':
						if (!fieldOpts.store)
							fieldOpts.store = Nuance.stores[name];
	...
	RouterUserPreferencesActivity: function(o) {
		...
		for (var n = 0; n < sections.length; n++) {
			...
			var field = new Nuance.TextLabel({
				text: _('optSection-' + i),
				target: self.body
			});
			field.index = -1;
	...
	ChatPopup: function(o) {
		...
		var historyField = ce('div', {
			className: 'message-area'
		}, this.body);
		var newMessageField = new Nuance.input.TextArea({
			title: '',
			target: this.body
		});
		var buttonGroup = new Nuance.input.ButtonGroup({
			buttons: [{
				value: _("Mark all as read"),
		...
		function markAllAsRead() {
			for (var i = 0; i < newMessages.length; i++) {
				...
				messageStore.edit(id, {
					is_new: 0,
					readdate: (new Date).toString(dbDateTimeFormat)
				});
			}
		}
	...
	StatsPopup: function(o) {
		...
		function loadContent() {
			...
			for (var i = 0; i < 4; i++) {
				...
				ce("p", {
					innerHTML: _("userstats-" + periods[i])
				}, part);
				var img = ce("img", {
					src: "../../ajax.php?action=getstatimage&path=" + path + "/" + periods[i] + extraParams,
		...
		for (var i = 0; i < o.store.header.length; i++) {
			...
			switch (htype) {
				...
				case 'speed':
					...
					break;
				case 'discount':
					field = new Nuance.input.DiscountField(fieldOpts);
					break;
				case 'bit':
				case 'tinyint':
					field = new Nuance.input.CheckBox(fieldOpts);
				...
				case 'tarifflist':
					field = new Nuance.input.TariffList(fieldOpts);
					break;
				case 'md5password':
					field = new Nuance.input.PasswordField(fieldOpts);
					break;
				case 'password':
					field = new Nuance.input.ViewPasswordField(fieldOpts);
					break;
				case 'generatepassword':
					field = new Nuance.input.GenerateViewPasswordField(fieldOpts);
					break;
				case 'date':
				case 'timestamp':
					...
					break;
				case 'phone':
					field = new Nuance.input.PhoneNumberField(fieldOpts);
					break;
				case 'charlink':
				case 'tarifflink':
				case 'multilink':
	...
	TabPanel: function(o) {
		var prefix = o.prefix ? o.prefix + '-' : '',
			self = this,
			tabPanelBody = this.tabPanelBody || ce('div', {
				id: prefix + 'tabpanel-body',
				className: 'tabpanel-body'
			}, o.target);
		var tabPanelSwitch = this.tabPanelSwitch || ce('ul', {
			id: prefix + 'tabpanel-switch',
			className: 'tabpanel-switch'
		}, tabPanelBody);
		var tabPanelContent = this.tabPanelContent || ce('div', {
			id: prefix + 'tabpanel-content',
		...
		function createGroup(name) {
			var groupWrapEl = ce('li', {
				className: 'group'
			}, tabPanelSwitch);
			var groupTitleEl = ce('span', {
				innerHTML: _(name)
			}, groupWrapEl);
	...
	Store: function(o) {
      ...
      if (filterString) {
		...
		this.send = function(action, id, postData, cb) {
			...
			var beforeEvent = new Nuance.Event({
				type: "before" + action
			});
	...
	widget: {
		__Widget: function(o) {
			...
			this.addRow = function(text, value) {
				var adminPaymentsRow = ce('p', {
					className: 'widget-row'
				}, self.content);
				ce('span', {
					className: 'widget-option',
					innerHTML: _("Sum of cashier payments")
				}, adminPaymentsRow);
				var adminPaymentsValue = ce('span', {
		...
		PotentialPaymentsStatisticsWidget: function(o) {
			...
			function onLoad() {
				...
				if (userStore.getState() === 'loaded' && tariffStore.getState() === 'loaded') {
					...
					var count = ce('span', {
						className: 'widget-value',
						innerHTML: smoneyf(potentionalPayments)
					}, row);
				}
			}
			...
			function onLoad() {
				...
				if (userStore.getState() === 'loaded' && cityStore.getState() === 'loaded') {
					...
					for (var i = 0; i < order.length; i++) {
						...
						var title = ce('span', {
							className: 'widget-option',
							innerHTML: cityStore.getNameById(n)
						}, row);
						var count = ce('span', {
							className: 'widget-value',
							innerHTML: usersByCity[n] + ' (' + Math.round(usersByCity[n] / usersCount * 100) + '%)'
						}, row);
					}
				}
			}
			...
			function onLoad() {
				...
				if (userStore.getState() === 'loaded' && tariffStore.getState() === 'loaded') {
					...
					for (var i = 0; i < order.length; i++) {
						...
						var title = ce('span', {
							className: 'widget-option',
							innerHTML: tariffStore.getNameById(n)
						}, row);
						var count = ce('span', {
							className: 'widget-value',
							innerHTML: usersByTariff[n] + ' (' + Math.round(usersByTariff[n] / usersCount * 100) + '%)'
						}, row);
					}
				}
			}
		...
		UserStatisticsWidget: function(o) {
			...
			var onlineUsersCountRow = ce('p', {
				className: 'widget-row'
			}, self.content);
			var onlineUsersCountTitle = ce('span', {
				className: 'widget-option',
				innerHTML: _("Number of online subscribers")
			}, onlineUsersCountRow);
			var onlineUsersCountRoutersCount = ce('span', {
				className: 'widget-suboption'
			}, onlineUsersCountTitle);

			var onlineUsersCountValue = ce('span', {
				className: 'widget-value'
			}, onlineUsersCountRow);
			...
			function onRouterStatesLoad() {
				...
				if (typeof routerStateChecker.data != 'undefined') {
	...
	Grid: function(opts) {
		...
		//grid body                 
		var self = this;
		var coverWrap = ce('div', {
			id: opts.name + '-cover-wrap',
		...
		var coverTextWrap = ce('p', {
			className: 'cover-text-wrap'
		}, cover);
		var coverText = ce('span', {
			className: 'cover-text'
		}, coverTextWrap);
		var name = opts.name;
		var gridState, filters = opts.filters || [],
			sortname, sortdesc, waitForStores = opts.waitForStores || [],
		...
		toolbarButtons.push({
			onclick: self.store.load,
			iconClass: 'reload',
			scope: self
		});
		var toolbarEl = ce('div', {
			className: 'toolbar'
		}, this.el);
		//grid toolbar
		var filtersWrapEl = ce('div', {
			...
			buttons = [],
			gridTableHeader = ce('div', {
				className: 'header'
			}, self.el),
			gridTable = ce('div', {
				className: 'table selectable'
			}, this.el),
			statusBarWrap = ce('div', {
				className: 'statusbar-wrap'
			}, toolbarEl),
			statusBar = ce('span', {
				className: 'list statusbar'
			}, statusBarWrap),
		...
		this.addButton = function(btn) {
			...
			{
				buttons = buttons.concat((new Nuance.input.ButtonGroup({
					noWrap: true,
					buttons: btn,
					target: toolbarEl,
					mergeButtons: true
		...
		var sort = function() {
			...
			if (self.store.sortOrder[hname]) {
				...
				if (opts.sorters && opts.sorters[htype]) {
					...
					dataOrder.sort(function(a, b) {
						return (self.displayData[a][index] > self.displayData[b][index]) ? 1 : -1
					});
				}
			}
    ...
    this.setRemoteFilters = function(newFilters) {
		...
		var searchBar = new Nuance.input.SearchField({
			target: toolbarEl,
		...
		var filterButton = new Nuance.input.Button({
			target: toolbarEl,
			iconClass: "filter",
			onlyIcon: true,
		...
		function createRow() {
			...
			for (var d = 0; d < self.displayData[anyProp].length; d++) {
				ce('span', {
					className: 'table-cell ' + self.displayHeader[d][0]
				}, gridTableRow);
			}
		}
		...
		function renderRows() {
			if (gridTable.offsetHeight) {
				...
				if (endEl > dataOrder.length)
					endEl = dataOrder.length;
			} else {
				var endEl = 0;
			}
		...
		this.render = function() {
			...
			if (dataOrder.indexOf(lastSelectedItem) !== -1) {
				...
				if (selectedRow && gridTable.scrollTop + gridTable.clientHeight + 80 < selectedRow.offsetTop)
