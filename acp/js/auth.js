...
if (!isIE)
{
  function showLoginPopup()
  {
    ...
    var form = ce('form', {className: 'login-body', action: './auth.php', method: "POST", autocomplete: 'on'}),
      actionField=ce('input', {name: 'action', value: 'login', type: 'hidden'}, form),
      loginField = new Nuance.input.TextField({title: _('Login'), name: 'login', onkeypress: onEnterKeyPress}),
      passwordField = new Nuance.input.PasswordField({title: _('Password'), name: 'password', onkeypress: onEnterKeyPress});
    var submitButton=new Nuance.input.Button({value: _("Authorize"), onclick: function(){form.submit()}});
    ...
    if (location.hash=='#badpassword')
    {
      var errWin=new Nuance.Popup(
      {
        title: _('Error'),
        closable: false,
        body: ce("div", {innerHTML: _("Wrong username or password")})
      });
    ...
    {
      ce('div', {className: "error", innerHTML: _("Wrong username or password") }, form, true);
    }
